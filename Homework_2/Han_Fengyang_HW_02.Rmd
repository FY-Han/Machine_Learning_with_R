...
```{r, solution_01a}
df_low_high %>% ggplot(aes(x=x,y=y)) + 
  geom_point()
```

**Is the relationship linear or non-linear?**

*Answer :* It seems to be a linear relationship at the first glimpse, but due to the high noise, it could be either linear or non-linear.
...
rmse_total <- tibble(x = seq(1,9), rmse_value = c(rmse1,rmse2,rmse3,rmse4,rmse5,rmse6,rmse7,rmse8,rmse9))

rmse_total %>% ggplot(aes(x=x)) +
  geom_point(aes(y=rmse_value)) + 
  geom_line(aes(y =rmse_value))
```

```{r}
df_rmse <- bind_rows(x=seq(1,9),rmse_value = c(rmse1,rmse2,rmse3,rmse4,rmse5,rmse6,rmse7,rmse8,rmse9))
```

```{r}
df_rmse %>% ggplot(aes(x=x)) +
  geom_point(aes(y=rmse_value)) + 
  geom_line(aes(y =rmse_value))
```
...
```{r, solution_01d}
r_squared_1 = modelr::rsquare(fit_lm_1,df_low_high)
r_squared_2 = modelr::rsquare(fit_lm_2,df_low_high)
r_squared_3 = modelr::rsquare(fit_lm_3,df_low_high)
r_squared_4 = modelr::rsquare(fit_lm_4,df_low_high)
r_squared_5 = modelr::rsquare(fit_lm_5,df_low_high)
r_squared_6 = modelr::rsquare(fit_lm_6,df_low_high)
r_squared_7 = modelr::rsquare(fit_lm_7,df_low_high)
r_squared_8 = modelr::rsquare(fit_lm_8,df_low_high)
r_squared_9 = modelr::rsquare(fit_lm_9,df_low_high)

r_squared_total <- tibble(x = seq(1,9), r_squared_value = c(r_squared_1,r_squared_2,r_squared_3,r_squared_4,r_squared_5,r_squared_6,r_squared_7,r_squared_8,r_squared_9))

ggplot(r_squared_total,aes(x=x)) +
  geom_point(aes(y=r_squared_value)) +
  geom_line(aes(y=r_squared_value))
...
The 9th degree model is the best. According to the RMSE and R-squared result, the higher polynomial would have lower RMSE and higher R-squared result, indicating a "better" model. The two metrics agree on the best model.

However, this is not reliable. As we learned in class, the performance of model should be the prediction ability on new data set, instead of training set.
...
Some feautres share common estimations, such as I(x \^ 3) and intercept. However, not all coefficients were estimated in the same way, such as in I (x \^ 4).

The level of uncertainty is also not consistent. For the 6th and 9th model, they both consider the 2nd polynomial statistically insignificant, while the 2nd model does.
...
The 9th model has predicted a sudden peak between the last two data point. However, we don't even have data point on the peak. This prediction therefore may not be correct, which require further evaluation.
...
```{r, solution_03b}
df_train %>% ggplot(aes(x=x)) + 
  geom_point(aes(y=y),shape=2,color="red",size=1) +
  geom_point(data=df_holdout,aes(x=x,y=y),shape=0,color="navy",size=1)
```
...
```{r, solution_03d}
rmse_fit_train_1 <- modelr::rmse(fit_train_1,df_holdout)
rmse_fit_train_1
```

```{r}
rmse_fit_train_2 <- modelr::rmse(fit_train_2,df_holdout)
rmse_fit_train_2
```

```{r}
rmse_fit_train_3 <- modelr::rmse(fit_train_3,df_holdout)
rmse_fit_train_3
```

```{r}
rmse_fit_train_4 <- modelr::rmse(fit_train_4,df_holdout)
rmse_fit_train_4
```

```{r}
rmse_fit_train_5 <- modelr::rmse(fit_train_5,df_holdout)
rmse_fit_train_5
```

```{r}
rmse_fit_train_6 <- modelr::rmse(fit_train_6,df_holdout)
rmse_fit_train_6
```

```{r}
rmse_fit_train_7 <- modelr::rmse(fit_train_7,df_holdout)
rmse_fit_train_7
```

```{r}
rmse_fit_train_8 <- modelr::rmse(fit_train_8,df_holdout)
rmse_fit_train_8
```

```{r}
rmse_fit_train_9 <- modelr::rmse(fit_train_9,df_holdout)
rmse_fit_train_9
```

```{r}
rmse_fit_train_total <- tibble(x = seq(1,9), rmse_value = c(rmse_fit_train_1,rmse_fit_train_2,rmse_fit_train_3,rmse_fit_train_4,rmse_fit_train_5,rmse_fit_train_6,rmse_fit_train_7,rmse_fit_train_8,rmse_fit_train_9))

rmse_fit_train_total %>% ggplot(aes(x=x)) +
  geom_point(aes(y=rmse_value)) +
  geom_line(aes(y=rmse_value))
```
...
The second quadratic model is the best with lowest RMSE value of 7.88.

It is not.

The model ranked worst by the hold-out set, which was identified as the best according to the training set.
...
The two models are very similar by sharing many coefficient with similar coefficient estimation and data range. One difference is that the 4th and 6th degree estimations are different in sign estimation.
...
The two models are very similar to each other except that the model trained on the split data has the 1st degree coefficient statisticallly insignificant.
...
my_ctrl <- caret::trainControl(method="cv",
                               number = 5, 
...
Accuracy, Kappa, and ROC are metrics for categorical response.

R-squared would increased with sample size, but are not suitable for sample size smaller than 10. Therefore, it has many restrictions.

Compared with MAE, RMSE is more sensitive to large deviations, which is a good metrics to tell differences between models.
...
The cubic model has the lowest max RMSE value of 6.31, which is the best.
...
Not all coefficient estimations are similar, such as for the coefficient of linear feature, the cubic model predict its value to be negative, while the others predict it to be positive. But for the others , they are quite similar.

The level of uncertainty is consistent, but not exactly consistent when estimates the x coefficient. The cubic and linear model predict its to be statistically insignificant, but the quadratic model predict it to be statistically significant.
...
```{r, solution_05a}
### your code here
df_low_low %>% ggplot(aes(x = x)) +
  geom_point(aes(y=y))
```

How does it compare?

This low noise data set showed a parabola relation between x and y, which is a non-linear relation.
...
In this fitting, the 6th degree model proves to be the best.
...
No, the best model is different in two cases.

The 9th degree model was identified to be poor for high noise case, and the same in low noise case.However, the 6th degree model which was about middle in high noise case was identified the best in the low noise case.
...
```{r, solution_06a}
### your code here
df_high_high %>% ggplot(aes(x = x)) +
  geom_point(aes(y=y))
```

How does it compare?

Although with high noise, we can still figure out a shape of non-linear relationship compared with the high noise data with less data point in 1a).
...
The 5th degree model is the best.
...
They didn't identify the same model as the best. but the 4th and 6th degree models both rank top in the two models.

The 9th model identified to be poor in both cases. The fifth model which was about middle quality in small sample size case was identified as the best in the large sample size case.
...
With large noise , we should increase the sample size, therefore the result would be similar with low noise and small sample size. Increasing the sample size or decrease noise could have a better shape of results in preview. It is better to have a lower noise data for discovery, since we can have much lower RMSE values in prediction.

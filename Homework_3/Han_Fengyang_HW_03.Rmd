...
Yes, it is. The event and non-event are about 50-50.
...
```{r, solution_01d}
### your code here
model_pred_df %>% ggplot(aes(x = x,y=y),size=3.5,alpha=0.5) + 
  geom_point()
```
...
```{r, solution_03f}
### your code here
model_pred_df %>% ggplot(aes(x = x, y = pred_prob)) +
  geom_line(color = "red" ) +
...
FP <- sum(model_class_0.5$obs_class == "non_event" & model_class_0.5$pred_class == "event")
  
FN <- sum(model_class_0.5$obs_class == "event" & model_class_0.5$pred_class == "non_event")
...
FP <- sum(model_class_0.7$obs_class == "non_event" & model_class_0.7$pred_class == "event")
  
FN <- sum(model_class_0.7$obs_class == "event" & model_class_0.7$pred_class == "non_event")

Accuracy_07 <- (TP + TN) / (TP + TN + FP + FN)
Accuracy_07
```
...
FP <- sum(model_class_0.3$obs_class == "non_event" & model_class_0.3$pred_class == "event")
  
FN <- sum(model_class_0.3$obs_class == "event" & model_class_0.3$pred_class == "non_event")

Accuracy_03 <- (TP + TN) / (TP + TN + FP + FN)
Accuracy_03
```
...
roc_data <- tibble (
  FPR = c(1-Specificity_03,1-Specificity_05,1-Specificity_07),
  Sensitivity = c(Sensitivity_03,Sensitivity_05,Sensitivity_07)
)
```
...
### model 7
set.seed(2021)
mod_7_acc <- train(y ~ x2 + x3 + I(x2^2) + I(x3^2),
                   data = df,
                   method = "glm",
                   metric = metric_acc,
                   preProcess = c("center","scale"),
...
We can not conclude which model is the best, because they all have wide confidence interval, however, model 3 has the highest mean accuracy value.
...
The Top2 has lower False positive values compared with the worst 2. Although model 8 has the lowest average accuracy, it's false positive value is lower than the model 4.

The Top2 also has lower False Negative values. Model 2 has an even lower value compared with Model 3.
...
```{r, solution_05a}
ctrl_roc <- trainControl(method = "cv",
                         number=5,
...
True positive rate (TPR) is Sensitivity, False-Positive rate (FPR) is 1-Specificity.

Model 2 has the highest True-Positive Rate at the default threshold and Model 2 has the lowest False-Positive Rate. However, Model 3 has the best ROC performance.
...
```{r, solution_05e}
### add your code here
tibble_mod_1_roc <- mod_1_roc$pred %>% tibble::as_tibble()
tibble_mod_1_roc %>% roc_curve(obs,event)

```

What do you think?

This is because threshold is low enough, the model would predict everything or almost everything to be "event", and no or very low false prediction. $$
Sensitivity = \frac{TP}{TP} = 1
$$
...
### add your code here
tibble_mod_1_roc <- mod_1_roc$pred %>% tibble::as_tibble()
tibble_mod_1_roc %>% roc_curve(obs,event) %>% autoplot()
```
...
The 3rd model is the best, which is consistent in the ROC prediction in previous results.

The 8th model is closest to a completely ineffective model,and very close to the diagonal, which means it is similar to random guess.
...
For every row, the value in the two columns add up to 1.
...
```{r, solution_06f}
### your code here
test_df_1_b %>% ggplot(aes(x=pred_bin)) + 
  geom_bar(aes(fill = y)) +
  scale_color_brewer(palette = 'Set1')
```

Yes, for the (0.9,1], it only contains event. None of them consist only non-events.
...
### your code here
test_df_1_b %>% ggplot() + 
  geom_bar(aes(x=pred_bin,fill = y),position="fill") +
  scale_color_brewer(palette = 'Set1')
```

No, they are not consistent in many bins.
...
my_calcurve_1 <- test_df_1_b %>%
  group_by(pred_bin)%>%
  summarise(event = sum(y == 'event'),
    non_event = sum(y == "non_event"),
    prop_event = event / (event+non_event)
  ) %>%
  select(-event,-non_event)

my_calcurve_1 <- my_calcurve_1 %>% mutate(mid_bin = seq(0.05,1,0.1))
```
...
```{r, solution_07b}
### your code here
my_calcurve_1 %>% ggplot(aes(x = mid_bin,y = prop_event)) + 
  geom_line() + 
  geom_point() + 
...
my_calcurve_3 <- my_calcurve_3 %>% mutate(mid_bin = seq(0.05,1,0.1))
...
my_calcurve_8 <- my_calcurve_8 %>% mutate(mid_bin = seq(0.35,0.8,0.1))
...
```{r, solution_08c}
### your code here
my_calcurve_3 %>% ggplot(aes(x = mid_bin,y = prop_event)) + 
  geom_line() + 
  geom_point() + 
  geom_abline(slope = 1,intercept = 0, color = 'grey', linetype = 'dashed') + 
  coord_equal(xlim = c(0,1),ylim = c(0,1))
...
```{r, solution_08d}
### your code here
my_calcurve_8 %>% ggplot(aes(x = mid_bin,y = prop_event)) + 
  geom_line() + 
  geom_point() + 
  geom_abline(slope = 1,intercept = 0, color = 'grey', linetype = 'dashed') + 
  coord_equal(xlim = c(0,1),ylim = c(0,1))
...
Yes, the ideal prediction should fit the diagonal, which means the prediction corresponds to the observed probability.

The Model 8 only captures event with probability from (0.3,0.8], which is quite a small range and low performance. The two other models cover the range from almost (0,1].
...
```{r, solution_09a}
### your code here
calibration(y ~ event, data = test_df_1, cuts=10) %>% 
  xyplot()
...
```{r, solution_09b}
### your code here
calibration(y ~ event, data = test_df_3, cuts=10) %>% 
  xyplot()
```
...
```{r, solution_09c}
### your code here
calibration(y ~ event, data = test_df_8
, cuts=10) %>% 
  xyplot()
```

Yes. But it assign value 0 to the datapoints not included.

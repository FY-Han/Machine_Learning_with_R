...
They are same. This means in a Bernoulli distribution, we can use mean value to describe the MLE.
...
\begin{align}
    &log[p(\mathbf{x} | \mu)] \\
    &= log(\prod^n_{n=1} (\mu^{x_n}(1-\mu)^{1-x_n}) \\
    &= \sum^n_{n=1} \left(log[\mu^{x_n}(1-\mu)^{1-x_n}]\right)\\
    &= \sum^n_{n=1} (x_nlog[\mu]+ (1-x_n) log [1-\mu])\\
    &= \sum^n_{n=1} (x_nlog[\mu]) + \sum^n_{n=1}((1-x_n) log [1-\mu])\\
    &= log[\mu]\sum^n_{n=1} (x_n) + log [1-\mu]\sum^n_{n=1}(1-x_n)\\
    &= log[\mu] \mathbf{M} + log[1-\mu]( \mathbf{N-M})
\end{align}
$$
...
$$
log[p(\mathbf{x} | \mu)]  = log[\mu](x_n) + log [1-\mu](1-x_n)\\
$$
...
log_bernoulli_pmf <- function(xobs, prob)
{
 ...
 for (i in xobs){
    prob_i <- log(prob) * i + log(1-prob) * (1-i)
    
    pmf_array <- append(pmf_array,prob_i)
 }
...
```{r}
print(log_bernoulli_pmf(x[1],0.250))
print(log_bernoulli_pmf(x[2],0.250))
print(log_bernoulli_pmf(x[3],0.250))
print(log_bernoulli_pmf(x[4],0.250))
print(log_bernoulli_pmf(x[5],0.250))
print(log_bernoulli_pmf(x[6],0.250))
```
...
```{r}
print(log_bernoulli_pmf(x[1:6],0.250))
```

Result length is 6. They are same compared with previous question.
...
$$
log[p(\mathbf{x} | \mu)] = log[\mu]\sum^n_{n=1} (x_n) + log [1-\mu]\sum^n_{n=1}(1-x_n)
$$
...
```{r}
log(0.25) * sum(x == 1) + log(1-0.25) * (length(x) - sum(x==1))
```

```{r}
sum(log_bernoulli_pmf(x,0.250))
```
...
```{r}
xa_results %>% ggplot(aes(x = mu)) + 
  geom_line(aes(y = log_lik))
```
...
$$ 
\log \left( p \left( H \mid AB, \mu \right) \right) \propto Hlog[\mu] + (AB-H)log[1-\mu]
$$
...
tibble::tibble(
  ...
  geom_line(size=1.1) +
  geom_vline(xintercept =μ ,size=1,linetype='dashed',color='red')
```
...
They share same shapes. But the dbinom graph has higher log_lik values, since it calculates the exact log_lik result instead of proportional result by including the constant.
...
```{r}
print(a_uniform + b_uniform)
print(a_inform + b_inform)
```
...
```{r}
print(a_uniform / (a_uniform + b_uniform))
print(a_inform / (a_inform + b_inform))
```
...
The uniform one show no preference on any mu value, while the inform one would have the largest probability near 0.23.

The uniform one didn't rule out any value since they are uniformed. The inform one, however, ruling out low possibilities of mu value near 0.05 and larger than 0.45.
...
$$ 
\log \left( p \left( m \mid N, \mu \right) \right) \propto  mlog[\mu] + (N-m) log[1-\mu]
$$
...
$$
\begin{align}
& log(p(\mu | a_{new},b_{new}))\\
& \propto log (p (m \mid N, \mu )) + log(p(\mu | a,b))\\
& \propto (m+a-1)log[\mu] + (N-m + b-1)log[1-\mu]
\end{align}
$$
...
$$
a_{new} = m+a, b_{new} = N-m+b
$$

We then have,

$$
log(p(\mu | a_{new},b_{new})) \propto (a_{new}-1)log[\mu] + (b_{new} -1)log[1-\mu]
$$
...
$$ 
\begin{align}
& \frac{d}{d[\mu]} [(a_{new}-1)log[\mu] + (b_{new} -1)log[1-\mu]]\\
...
new_ab <- tibble(
  name = c("uniform","inform"),
  a_new = c(a_new_uniform,a_new_inform),
  b_new = c(b_new_uniform,b_new_inform)
)
...
new_ab %>% mutate(
  ...
  quantile_05 = qbeta(0.05,a_new,b_new),
  quantile_95 = qbeta(0.95,a_new,b_new)
) %>% 
  select(mean,mode,quantile_05,quantile_95)
...
The first one barely changes the mode, while the second one has significantly changes the mode value given more prior knowlege. The possibility also gets higher at the new mode.
...
```{r}
a_new_season_uniform <- a_uniform + season_hits
b_new_season_uniform <- b_uniform + season_atbats - season_hits
a_new_season_inform <- a_inform + season_hits
b_new_season_inform <- b_inform + season_atbats - season_hits

new_season_ab <- tibble(
  name = c("uniform","inform"),
  a_new = c(a_new_season_uniform,a_new_season_inform),
  b_new = c(b_new_season_uniform,b_new_season_inform)
)
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu,a_new_season_uniform,b_new_season_uniform) ) %>%
  ggplot(aes(x=mu,y=beta_pdf)) + 
  geom_line()
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu,a_new_season_inform,b_new_season_inform) ) %>%
  ggplot(aes(x=mu,y=beta_pdf)) + 
  geom_line()
```
...
Even though two prior models were different at beginning, given more observed sample data, the posteriors become similar in all aspect. Therefore, if the observed data sample size is small, the posterior would be largely influenced by prior parameters. If the observed data sample size is large enough, the parameters would suffer little influence by prior parameter estimations.

...
```{r}
print(qnorm(p = 0.25,mean = 125,sd = 5))
print(qnorm(p = 0.75,mean = 125,sd = 5))
```
...
```{r}
print(pnorm(q = 117,mean = 125,sd = 5))
```

The probability that mean performance $\mu$ is less than 117 is 0.0547
...
Given $\sigma = 5$, $\tau_0 = 5$ , $\tau_N = 2$

$$
N = 5.25
$$

Given $\sigma = 5$, $\tau_0 = 5$ , $\tau_N = 0.5$
...
Given $\sigma = 15$, $\tau_0 = 5$ , $\tau_N = 3$

$$
N = 16
$$

Given $\sigma = 15$, $\tau_0 \rightarrow \infty$ , $\tau_N = 3$
...
$$ 
\begin{align}
log \left[p\left(\mu \mid \mathbf{x}, \sigma\right) \right] &\propto\sum_{n=1}^{N}\left\{\log \left[\operatorname{normal}\left(x_{n} \mid \mu, \sigma\right)\right]\right\}+\log \left(\operatorname{normal}\left(\mu \mid \mu_{0}, \tau_{0}\right)\right) \\
&\propto \sum_{n=1}^{N}\left\{\log \left[\frac{1}{\sqrt{2 \pi \sigma^{2}}} \exp \left(-\frac{1}{2 \sigma^{2}}\left(x_{n}-\mu\right)^{2}\right)\right]\right\}+\log \left[\frac{1}{\sqrt{2 \pi \tau_{0}^{2}}} \exp \left(-\frac{1}{2 \tau_{0}^{2}}\left(\mu-\mu_{0}\right)^{2}\right)\right] \\
&\propto \sum_{n=1}^{N}\left(-\frac{1}{2 \sigma^{2}}\left(x_{n}-\mu\right)^{2}\right)+\left(-\frac{1}{2 \tau_{0}^{2}}\left(\mu-\mu_{0}\right)^{2}\right) \\
&\propto - \frac{1}{2 \sigma^{2}}\sum_{n=1}^{N} \left(x_{n}-\mu\right)^{2} -\frac{1}{2 \tau_{0}^{2}}\left(\mu-\mu_{0}\right)^{2} \\
\end{align}
$$
...
$$
\begin{align}
\frac{d}{d\mu} \left( log \left[p\left(\mu \mid \mathbf{x}, \sigma\right) \right] \right) & = \frac{d}{d\mu} \left( - \frac{1}{2 \sigma^{2}}\sum_{n=1}^{N} \left(x_{n}-\mu\right)^{2} -\frac{1}{2 \tau_{0}^{2}}\left(\mu-\mu_{0}\right)^{2} \right) \\
& = \frac{1}{\sigma^2} \sum_{n=1}^{N}(x_n - \mu) - \frac{1}{\tau_0^2}(\mu - \mu_0) \\
& = \frac{1}{\sigma^2} (N\bar x_n - N\mu) - \frac{1}{\tau_0^2}(\mu - \mu_0) \\
& = \frac{N}{\sigma^2} (\bar x - \mu) - \frac{1}{\tau_0^2}(\mu - \mu_0) \\
\end{align}
$$
...
$$
\begin{align}
\frac{N}{\sigma^2} (\bar x - \mu) - \frac{1}{\tau_0^2}(\mu - \mu_0) &= 0 \\
(\frac{N}{\sigma^{2}}+\frac{1}{\tau_{0}^{2}}) \mu & =\frac{N}{\sigma^{2}} \bar{x}+\frac{1}{\tau_{0}^{2}} \mu_{0}\\
\mu &= \mu_{MAP} = \frac{\frac{N}{\sigma^{2}} \bar{x}+\frac{1}{\tau_{0}^{2}} \mu_{0}}{\frac{N}{\sigma^{2}}+\frac{1}{\tau_{0}^{2}}}
\end{align}
$$
...
The mean value $\mu_N$ of posterior distribution on $\mu$ given $\mathbf{x}$ and $\sigma$

This is because we applied Normal Distribution for prior, which is the same hypothesis as we discussed in class, and the posterior is a gaussian distribution. The mean and mode(MAP) value for Gaussian distribution is the same.
...
$$
\begin{align}
\log [p(\sigma \mid \mathbf{x}, \mu)] &\propto \sum_{n=1}^{N}\left\{\log \left[\mathrm{normal}\left(x_{n} \mid \mu, \sigma\right)\right]\right\}+\log [\mathrm{Exp}(\sigma \mid \lambda)] \\
&\propto \sum_{n=1}^{N} \left\{ \log \left[ \frac{1}{ \sqrt{2\pi\sigma^2} } \exp \left( -\frac{1}{2\sigma^2} (x_n - \mu)^2 \right)  \right] \right\} + \log \left[  \lambda \exp \left(-\lambda \sigma \right) \right] \\
&\propto -\sum_{n=1}^{N}\log\sigma-\frac{1}{2\sigma^2} \sum_{n=1}^{N}(x_n - \mu)^2 - \lambda\sigma \\
&\propto -\frac{N}{2}\log\sigma^2 - \frac{1}{2\sigma^2}\sum_{n=1}^{N}(x_n - \mu)^2 - \lambda\sigma \\
\end{align}
$$
...
$$
\log [p(\sigma \mid \mathbf{x}, \mu)] \propto -\frac{N}{2}\log\sigma^2 - \frac{vN}{2\sigma^2} - \lambda\sigma
$$

$v$ includes all information from sample data, therefore, it is called as "sufficient".

The expression focused on the general performance instead of individual observations, and we only need the general performance statistic $v$.
...
{
  # your code here
  logp_unnorm = -N/2*log(sigma^2) - v*N/(2*sigma^2) - lambda*sigma
...
tibble::tibble(
  ...
  ggplot(aes(x = sigma)) + 
  geom_line(aes(y = log_post),size=1)
```
...
It is not symmetric so it doesn't look like a parabola.
...
\frac{d}{d\varphi} \left( g^{-1}\left(\varphi\right) \right) &= \frac{d}{d\varphi}e^{\varphi} = e^{\varphi}\\
\log\left( \left | \frac{d}{d\varphi} \left( g^{-1}\left(\varphi\right) \right) \right | \right) &= \log[e^{\varphi}] = \varphi
\end{align}
$$
...
tibble::tibble(
  varphi = log(seq(0.5,4.5,length.out = 101))
) %>% 
...
It appears to be more parabolic now.
...
tibble::tibble(
  ...
  mutate(log_post = log_post_varphi_unnorm(varphi = varphi, N = N, v = v, lambda = 0.75)) %>% 
  ggplot(mapping = aes(x = varphi, y = log_post)) + geom_line(size = 1) + 

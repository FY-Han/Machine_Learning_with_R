...
hw06_df %>%
  summarise(mean(x),sd(x),min(x),max(x))
...
param_grid %>% 
  ...
  geom_point(data = tibble::tibble(xbar = sample_average, xsd = sample_sd),
...
The $\sigma$ is close to posterior mode which represents by the inner region. But the $\mu$ value is far away from the posterior mode by locating outside the top 50% posterior combination represented by the second ring region.

For the posterior uncertainty of $\mu$ and $\sigma$, we may state that when $\mu$ grows, the uncertainty of $\sigma$ increases first and then decreases. Furthermore, an uncertainty of $\sigma$ less than the posterior mean seems little, but its range expands dramatically if it is significant.
...
$$
\begin{align}
\frac{d}{d\mu} \{ \log \left[ p \left( \mathbf{x} \mid \mu, \sigma \right) \right] \} &= 
  \frac{N}{\sigma^2} (\bar{x} - \mu) - \frac{1}{\tau_{0}^2} (\mu - \mu_{0}) \\
\frac{d^2}{d \mu^2} \{\log \left[ p \left( \mathbf{x} \mid \mu, \sigma \right) \right] \} &= 
  \frac{d}{d \mu} \left[ \frac{N}{\sigma^2} ( \bar{x} - \mu ) \right] - 
  \frac{d}{d \mu} \left[ \frac{1}{\tau_{0}^2} ( \mu - \mu_0 ) \right]\\
...
The first derivative equals to 0, and the second derivative is negative, so $\mu$ associates with the maximum.
...
$$
\mathrm{m}_N = \mu_{MAP} = 
  \frac{ \frac{N}{\sigma^2} \bar{x} + \frac{1}{\tau_{0}^2} \mu_{0} } { \frac{N}{\sigma^2} + \frac{1}{\tau_{0}^2}}
$$ where $\bar{x}$ is the sample average.

The posterior variance is the inverse of negative Hessian matrix, thus $$
\mathrm{s}_N^{2} = 
  \left\{ - \frac{d^2}{d \mu^2} \{\log \left[ p \left( \mathbf{x} \mid \mu, \sigma \right) \right] \} \right\} ^ {-1} = 
  \left( \frac{N}{\sigma^2} + \frac{1}{\tau_{0}^2} \right) ^ {-1}
$$
...
The Normal Prior conjugates with Normal Posterior, so the posterior is also normal distribution. Laplace Approximation tries to fit a normal distribution for the posterior. Therefore, the Laplace Approximation is the same as the exact posterior distribution for this specific application.
...
Lower bound for noise $\sigma$ is 0, while the normal distribution doesn't have a lower bound. The log transformation cancel the lower bound for the distribution and overcome the limitation.
...
$$
p \left( \mu, \varphi \mid \mathbf{x} \right) \propto \prod_{n=1}^{N} \left( \mathrm{normal} \left(x_n \mid \mu, g^{-1} (\varphi) \right) \right) \times \mathrm{normal}\left(\mu \mid \mu_0, \tau_0\right) \times \mathrm{Exp}\left(g^{-1} (\varphi) \mid \lambda\right) \times \left| \frac{d}{d\varphi} \left( g^{-1}\left(\varphi\right) \right) \right|
$$`<!-- -->`{=html}3c)
...
\log \left\{ \frac{d}{d\varphi} \left[ g^{-1}(\varphi) \right] \right\} = \log \left[ \exp(\varphi) \right] = \varphi
$$`<!-- -->`{=html}3d)
...
log_post_cv_result <- purrr::map2_dbl(cv_param_grid$mu, cv_param_grid$varphi,eval_logpost,logpost_func = my_cv_logpost,logpost_info = hw06_info)
```
...
cv_param_grid %>% 
  ...
  geom_point(data = tibble::tibble(xbar = sample_average, log_xsd = log(sample_sd)),
...
They have very little difference.

The identified optimal parameter values and the Hessian matrices the same.

One difference is the number of iterations.

The log-posterior surface suggests there is only one optimal point, so no wonder the two guesses converge to the same answer.
...
my_laplace <- function(start_guess, logpost_func, info)
{
  ...
  int <- p/2 * log(2 * pi) + 0.5 * log(det(post_var_matrix)) + logpost_func(mode, info)
...
laplace_result <- my_laplace(start_guess = init_guess_01, 
...
```{r}
post_mean <- laplace_result$mode[1]
post_mean
```
...
```{r}
post_mean_sd <- sqrt(laplace_result$var_matrix[1,1])
post_mean_sd
```

```{r}
post_varphi_sd <- sqrt(laplace_result$var_matrix[2,2])
post_varphi_sd
```

```{r}
laplace_result$var_matrix[1,2] / (post_mean_sd * post_varphi_sd)
```
...
```{r}
post_samples %>% 
  ggplot(mapping = aes(x = mu)) + geom_histogram(bins = 55)
```

```{r}
post_samples %>% 
  ggplot(mapping = aes(x = sigma)) + geom_histogram(bins = 55)
```
...
```{r}
post_sample_sigma <- post_samples$sigma

length(post_sample_sigma[post_sample_sigma > 4]) / 10000
```

10.84% that the noise is greater than 4 units.

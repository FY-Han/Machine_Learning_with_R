...
```{r}
glimpse(Xmat_quad_big)
```

The Xmat_quad_small has 3 columns with 5 rows, and the Xmat_quad_big has 3 columns and 30 rows.

The ones are contained in the FIRST column, which corresponds to the intercepts. We make this intercept to have it easily handled in a matrix form.
...
```{r}
glimpse(covmat_quad_small_noprior)
```

```{r}
glimpse(covmat_quad_big_noprior)
```
...
Each posterior covariance matrix has 3 rows and 3 columns.
...
```{r}
post_small_std <- sqrt(diag(covmat_quad_small_noprior))
post_big_std <- sqrt(diag(covmat_quad_big_noprior))
...
postmeans_quad_small_noprior <-covmat_quad_small_noprior %*% t(Xmat_quad_small) %*% quad_data_small$y
...
postmeans_quad_big_noprior <-covmat_quad_big_noprior %*% t(Xmat_quad_big) %*% quad_data_big$y
...
The values will not change if the \sigma change
...
qnorm(c(0.025,0.975),postmeans_quad_small_noprior[3],post_small_std[3])
qnorm(c(0.025, 0.975), postmeans_quad_big_noprior[3], post_big_std[3])
```

What do you think?

They are significant, without containing 0.
...
```{r}
post_quad_std_small_inform <- sqrt(diag(covmat_quad_small_inform))
post_quad_std_big_inform <- sqrt(diag(covmat_quad_big_inform))

post_quad_std_small_inform
post_quad_std_big_inform
```

They are not similar on small sample size, but similar on large sample size.
...
```{r}
post_quad_std_small_vague <- sqrt(diag(covmat_quad_small_vague))
post_quad_std_big_vague <- sqrt(diag(covmat_quad_big_vague))

post_quad_std_small_vague
post_quad_std_big_vague
```
...
Small datasets could have reduced posterior uncertainty by introducing prior.
...
No, there is only one final result when converged.
...
The rows corresponds to the 30 predicted points, and 2500 generated posterior points.
...
The three MAE values are different. Becuase each sample here has different $\beta$ values.
...
The MAE is uncertain because of the uncertainty of the $\beta$ values. The the predicted values are uncertain, which makes the MAE is uncertain.
...
150 rows in the training design matrix and 50 in the test one.
...
Models with degree of freedom more than 7 overfit, with increasing MAE in the test set.
...
The 7th or the 9th model is the best.
...
Using single one is not a good idea. The conclusions from a single train/test split might be more susceptible to variability and overfitting, while evidence-based assessments often provide a more comprehensive and robust evaluation of model performance. The difference would then comes from the drawbacks of using a single train/test split, such as overfitting.

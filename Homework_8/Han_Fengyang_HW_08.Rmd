...
It is not clear to claim any of the features is statistically significant, with every one containing zero from observation.
...
4 features. Yes, the x1:x2 and x2 coefficients are statistically significant.
...
9 features. I(x2\^2) and intercept features are statistically significant.
...
The intercept feature is statistically significant.
...
glance_results <- data.frame()

for (i in seq_along(models)) {
  glance_output <- broom::glance(models[[i]])
  glance_results <- rbind(glance_results, glance_output)
}

glance_results
```

mod06 is the best according to the R-squared.

mod03 is the best according to AIC.

mod03 is the best according to BIC.
...
The higher complexity and overfitting are the characteristics.
...
length_beta_03 = length(laplace_03_weak$mode) - 1

post_means_03_weak <- laplace_03_weak$mode[1:length_beta_03]
post_sds_03_weak <- sqrt(diag(laplace_03_weak$var_matrix))[1:length_beta_03]

viz_post_coefs(post_means_03_weak, post_sds_03_weak, colnames(X03))
```

```{r, solution_03d_b}
### make the posterior coefficient visualization for model 6
length_beta_06 = length(laplace_06_weak$mode) - 1

post_means_06_weak <- laplace_06_weak$mode[1:length_beta_06]
post_sds_06_weak <- sqrt(diag(laplace_06_weak$var_matrix))[1:length_beta_06]

viz_post_coefs(post_means_06_weak, post_sds_06_weak, colnames(X06))
```
...
The Bayes factor of model 3 is much larger than model6, showing it to be a better model.
...
```{r, solution_03g}
### add more code chunks if you like
length_beta_03 = length(laplace_03_strong$mode) - 1

post_means_03_strong <- laplace_03_strong$mode[1:length_beta_03]
post_sds_03_strong <- sqrt(diag(laplace_03_strong$var_matrix))[1:length_beta_03]

viz_post_coefs(post_means_03_strong, post_sds_03_strong, colnames(X03))
```

```{r}
length_beta_06 = length(laplace_06_strong$mode) - 1

post_means_06_strong <- laplace_06_strong$mode[1:length_beta_06]
post_sds_06_strong <- sqrt(diag(laplace_06_strong$var_matrix))[1:length_beta_06]

viz_post_coefs(post_means_06_strong, post_sds_06_strong, colnames(X06))
```
...
```{r, solution_03i}
### add more code chunks if you like
length_beta_03 = length(laplace_03_very_strong$mode) - 1

post_means_03_very_strong <- laplace_03_very_strong$mode[1:length_beta_03]
post_sds_03_very_strong <- sqrt(diag(laplace_03_very_strong$var_matrix))[1:length_beta_03]

viz_post_coefs(post_means_03_very_strong, post_sds_03_very_strong, colnames(X03))
```

```{r}
length_beta_06 = length(laplace_06_very_strong$mode) - 1

post_means_06_very_strong <- laplace_06_very_strong$mode[1:length_beta_06]
post_sds_06_very_strong <- sqrt(diag(laplace_06_very_strong$var_matrix))[1:length_beta_06]

viz_post_coefs(post_means_06_very_strong, post_sds_06_very_strong, colnames(X06))
```
...
A larger prior standard deviation implies that we have less certainty about the value of the regression coefficients a priori. This will usually result in wider posterior distributions, indicating greater uncertainty about the estimated coefficients after seeing the data.With stronger prior (sd = 1/50), the coefficient posterior means are very close to 0 and the confidence interval is very small.
...
X06_test <- model.matrix( ~ splines::ns(x1, knots = x1_knots$interior_knots, Boundary.knots = x1_knots$boundary_knots) * 
                            (x2 + I(x2^2) + I(x2^3) + I(x2^4)), data = viz_grid)
...
As the prior standard deviation decreased, the coefficients are constrained more to the prior mean.
...
It looks like in both models, there are only coefficients remaining.
...
Two features were not turned off, the x2\^2 term and intercept.\
The final tuned models are the same, both of them only contains the x2\^2 term and the intercept.

...
```{r}
lfA_train %>% ggplot(aes(x = input_id, y = value))+
  geom_boxplot(aes(group=input_id))
```
...
lfA_train %>% filter(26 <= input_id & input_id < 50) %>%
  ggplot(aes(x = value, y = y)) + 
  geom_point(alpha=0.33) + 
  facet_wrap(facets = ~name)
```
...
They all seem to have little relation with the output.
...
dfA_train %>% select(starts_with('x')) %>%
  cor()%>%
  corrplot::corrplot()
```
...
No they are not. The color are all light except for the diagnol line, suggesting low correlation.
...
```{r}
check_info_A_tmp <- list(
  yobs = dfA_train$y,
  design_matrix = model.matrix(y ~ (x01+x02)^2, data = dfA_train),
  lambda = 1
)
```
...
loss_ridge <- function(betas, my_info)
{
  ...
  # calculate MSE
  MSE <- sum((my_info$yobs - mu) ^ 2) / length(mu)
...
fit_regularized_regression <- function(lambda_use, loss_func, my_info)
{
  ...
  start_guess <- c(rep(0, ncol(my_info$design_matrix)))
...
The ridge regularized coefficient are very close to zero this time.
...
lambda_grid <- 10 ^ (seq(-4,4,length.out=101))
```
...
ridge_path_results %>% 
  ...
  geom_line(aes(group = term), alpha = 0.33) 
```
...
As the regularization strength increases, the coefficients are getting closer to zero.
...
```{r}
loss_lasso(c(rep(-1.2, ncol(check_info_A$design_matrix))), check_info_A)
```

Test `loss_lasso()` with 0.2 for all regression coefficients.

```{r}
loss_lasso(c(rep(0.2, ncol(check_info_A$design_matrix))), check_info_A)
```
...
They are about the same with each other.
...
They are different. The lasso regularized coefficients are about 0.
...
They are different. the lasso paths converged to 0 abruptly, while the ridge paths gradually converged to 0.
...
train_and_test_regularized <- function(lambda_use, train_data, test_data, loss_func)
{
  ...
  MSE_test <- sum((test_data$y - mu_test) ^ 2) / length(mu_test)
...
```{r}
train_test_lasso_results %>% mutate(loglambda = log(lambda)) %>%
  ggplot() + 
  geom_line(aes(x = loglambda, y = MSE_test))
```

```{r}
result <- train_test_lasso_results %>% arrange(MSE_test)
exp(result$lambda %>% head(n=1))
```

Which $\lambda$ is the best?

lambda 8.079226 is the best
...
```{r}
viz_compare_to_mles(lasso_path_results %>% filter(lambda == result$lambda %>% head(n=1)) , modA)
```
...
Not all coefficients converge to zero at the same time as previous.
...
The best lasso model has 24 non-zero features. By the 1-standard error rule, the Lasso model recommended 4 non-zero features.
...
The input x03, x07, x21, and x33 matter based on the tuned lasso model.
...
The best parameter is 0.55 and lambda 1.25014580.

The best set are more consistent with Lasso beacause alpha \> 0.5.
...
```{r}
coef(enet_A$finalModel, s=enet_A$bestTune$lambda)
```

The non-zero coefficients: intercept, x03, x04, x06, x07, x10, x14, x16, x18, x21, x22, x24, x25, x27, x29, x33-35, x39, x42-44, x49, x51, x53, x54, x57, x61, x62, x65, x66, x69, x71, x75.
...
```{r}
lfB %>% ggplot(aes(x = value, y = value)) + 
  geom_point() + 
  facet_wrap(~name)
```
...
```{r}
dfB %>% cor() %>% corrplot::corrplot()
```

```{r}
dfB %>% cor() %>% corrplot::corrplot(type = 'upper')
```

```{r}
dfB %>% cor() %>% corrplot::corrplot(type='upper', method='color')
```
...
```{r}
dfB %>% cor() %>% corrplot::corrplot(type='upper', method='color',order='hclust', hclust.method = 'ward.D2')
```
...
The best tunning parameter combinations are alpha = 0.10 and lambda = 1.21631339.\
The best set of tuning parameters are more consistent with Ridge regression because alpha \< 0.5.

Yes, it seems to have impacts on the model behaviors.
...
Except for x5:x6, and x5:x7, all are non-zero.
...
Yes, they are. Except that the intercept is not displayed.
...
If a model is overfitted to the training data, the feature ranking may not generalize well.
...
```{r}
dfC %>% ggplot(aes(x = x, y = y)) + 
  geom_point(aes(color = v), size = 4)
```

Yes they are. The A and B have positive relations, C has a negative relation and D is not clear.
...
It corresponds to the expected value of the response variable
...
```{r}
modC_int <- lm(y ~ x + v + x * v, data = dfC)
...
```{r}
modC_add %>% coefplot::coefplot()
```

```{r}
modC_int %>% coefplot::coefplot()
```

The interaction model seems better, since it identifies x:vC as a statistically significant feature, which means at level C, the y is statistically significant to have negative correlation with x.

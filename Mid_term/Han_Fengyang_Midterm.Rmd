...
In the beta distribution, the overall prior trials number equals a + b = 1

It represents the "uninformed" since we only have one prior trails, which is a very small number, and provides very little information. The result of this binomial event could either be "event" and "non-event" and the observed result would provide no information on the probability.
...
Since the likelihood function is binomial and we have a beta prior, the posterior distribution would then be the conjugate binomial function
...
uninform_summary <- tibble::tibble(
  mean = a/(a+b),
  q0.05= qbeta(0.05,a,b),
  q0.95= qbeta(0.95,a,b)
)

uninform_summary
```

The middle 90% uncertainty interval is 0.0062 to 0.9938
...
post_df_focus_from_vague <- df_focus %>% mutate(
  anew = a + num_events,
...
summary_post_df_focus_from_vague <- post_df_focus_from_vague %>% mutate(
...
Yes, there are movies with MLE outside the posterior uncertainty, such as movie_id 10874 and 11123, with MLE value 0, and 1 respectively.

Yes, there are movies with posterior mean value close to MLE, such as movie_id 266 and 797.
...
summary_post_df_focus_from_vague %>% 
  ...
  geom_linerange(aes(ymin = post_q05,ymax=post_q95,group = movie_id),position = position_dodge(0.2)) + 
  geom_point(aes(y = post_avg,group = movie_id),position = position_dodge(0.2)) + 
  geom_point(aes(y = num_events / num_trials,group = movie_id),color="blue",position = position_dodge(0.2)) +
  theme(axis.text.x = element_text(angle = 45))
...
The movie with smaller num_trials has higher posterior uncertainty, which is 11423 and 4894.
...
```{r}
post_df_all_from_vague <- df_all %>% mutate(
  anew = a + num_events,
  bnew = b + num_trials - num_events
)
```
...
```{r}
summary_post_df_all_from_vague <- post_df_all_from_vague %>% mutate(
  post_avg = anew / (anew+bnew),
  post_q05 = qbeta(0.05,anew,bnew),
  post_q95 = qbeta(0.95,anew,bnew)
)
...
```{r}
glimpse(summary_post_df_all_from_vague)
```
...
summary_post_df_all_from_vague %>%
  ggplot(aes(x=post_avg))  + 
  geom_point(aes( y = num_trials / num_events,color=num_trials))+
...
```{r}
summary_post_df_all_from_vague %>%
  ggplot(aes(x = post_q95 - post_q05)) + 
  geom_point(aes(y = num_trials))+
  scale_x_log10() 
```
...
$$
\begin{align}
&\log \left( p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right)  \right)\\
&\propto \log \left( \prod_{j=1}^{J} \left( \mathrm{Binomial} \left(m_j \mid \mu, N_j \right) \right) \times \mathrm{Beta} \left(\mu \mid a, b\right) \right) \\
&\propto \sum_{j=1}^{J} \left( \log \left( \mu^{m_j} (1-\mu)^{N_j-m_j} \right) \right) + \log \left( \mu^{a-1} (1-\mu)^{b-1} \right)\\
&\propto \sum^J_{i=1}m_j \left( \log|\mu|  \right) + \sum^J_{i=1}(N_j - m_j)\left( log|1-\mu| \right) + (a-1)\log|\mu| + (b-1)\log|1-\mu| \\
&\propto log(|\mu|) \sum_{j=1}^{J} \left( m_j \right) + \log(|1-\mu|) \sum_{j=1}^{J} \left( N_j-m_j \right) + (a-1) \log(|\mu|) + (b-1) \log(|1-\mu|)
\end{align}
$$
...
&\propto ( J \bar{m} + a - 1 ) \log(\mu) + ( J \bar{N} - J \bar{m}+ b - 1) \log(1-\mu)
\end{align}
$$
...
$$
\mathrm{Beta} ( \mu \mid a + J \bar{m}, b + J \bar{N} - J \bar{m})
$$
...
post_df_focus_pooled <- df_focus %>% mutate(
    anew = a + sum(df_all$num_events),
    bnew = b + sum(df_all$num_trials) - sum(df_all$num_events)
)
```
...
Yes, there are many, due to very small uncertainty interval.

Yes, there are some, such as 3037.
...
Grouping everything together results in a very little uncertainty interval which is incorrect since it is should be case by case.
...
```{r}
df_all %>% 
  ggplot(aes(x = num_events / num_trials)) +
...
It becomes flat, with smaller count values.
...
ab_emp_bayes <- exp(c(log_ab_res_01$par[1], log_ab_res_01$par[2]))
sum(ab_emp_bayes)
```

How many a-priori trials?

About 5 a-priori trials.
...
```{r, solution_03g_b}
###
df_30 %>% ggplot(aes(x = num_events / num_trials)) + 
  geom_histogram(binwidth = 0.05,aes(y=after_stat(density))) +
  geom_line(data = prior_for_viz, aes(x = x, y = beta_pdf),color="red",linewidth=1.15)
...
ab_emp_bayes_2 <- c(4,2.5)

prior_for_viz_2 <- tibble::tibble(
  x = seq(min(info_for_ab$theta), max(info_for_ab$theta), length.out = 1001)
) %>% 
  mutate(beta_pdf = dbeta(x, 
                          ab_emp_bayes_2[1], 
...
df_30 %>% ggplot(aes(x = num_events / num_trials)) + 
  geom_histogram(binwidth = 0.05,aes(y=after_stat(density))) +
  geom_line(data = prior_for_viz_2, aes(x = x, y = beta_pdf),color="red",linewidth=1.15)
...
```{r}
ab_emp_bayes_summary <- tibble::tibble(
  mean = ab_emp_bayes[1] / sum(ab_emp_bayes), 
  q0.05 = qbeta(0.05, ab_emp_bayes[1], ab_emp_bayes[2]), 
  q0.95 = qbeta(0.95, ab_emp_bayes[1], ab_emp_bayes[2])
)
ab_emp_bayes_summary
```

```{r}
ab_emp_bayes_2_summary <- tibble::tibble(
  mean = ab_emp_bayes_2[1] / sum(ab_emp_bayes_2), 
  q0.05 = qbeta(0.05, ab_emp_bayes_2[1], ab_emp_bayes_2[2]), 
  q0.95 = qbeta(0.95, ab_emp_bayes_2[1], ab_emp_bayes_2[2])
)
ab_emp_bayes_2_summary
```
...
```{r}
ab_emp_bayes_summary$q0.95 - ab_emp_bayes_summary$q0.05
```

```{r}
prior_for_comp <- tibble::tibble(
  x_emp_bays = seq(min(info_for_ab$theta), max(info_for_ab$theta), length.out = 1001),
  x_uninfo = seq(0, 1, length.out = 1001)
) %>% 
  mutate(beta_emp_bays = dbeta(x_emp_bays, ab_emp_bayes[1], ab_emp_bayes[2]),
         beta_uninfo = dbeta(x_uninfo, 0.5, 0.5))

prior_for_comp %>% 
  ggplot() + 
  geom_vline(xintercept = c(ab_emp_bayes_summary$q0.05, ab_emp_bayes_summary$q0.95), color = "blue", linetype = "longdash") +
  geom_vline(xintercept = c(uninform_summary$q0.05, uninform_summary$q0.95), color = "red", linetype = "longdash")
```

The blue lines are the Empirical Bayes derived interval distribution, and the red lines are the original uninformed ones. The middle 90% uncertainty intervals of the uninformed prior is very wide while that of the Empirical Bayes derived informative prior is much smaller. This is because, compared to the uninformed prior, we used a lot of samples to estimate the prior shape parameters.
...
post_df_focus_empbayes <- df_focus %>% 
  mutate(anew = 4 + num_events, 
         bnew = 2.5 + num_trials - num_events)
```
...
We have MLEs outside the middle 90% uncertainty interval. This is different from the two priors under extreme scenarios.
...
summary_post_df_focus_empbayes %>% 
  ...
  geom_linerange(aes(ymin = post_q05, ymax = post_q95, group = movie_id), position = position_dodge(0.2)) + 
  geom_point(aes(y = post_avg, group = movie_id), position = position_dodge(0.2)) +
  geom_point(aes(y = num_events / num_trials, group = movie_id), color = "red", position = position_dodge(0.2))
```
...
summary_post_df_all_empbayes %>% 
  ggplot(aes(x = post_avg)) + 
  geom_point(aes(y = num_events / num_trials, color = num_trials > 30)) + 
  geom_abline(slope = 1, intercept = 0)
```
...
compare_post_summaries %>% 
  ggplot(aes(x =post_q95 - post_q05)) + 
  geom_point(aes(y = num_trials,color=from_prior)) + 
  scale_x_log10()
```
...
With a higher trial number, we have a smaller mideel 90% uncertainty interval range. When the sample size is small, the data provides limited information about the parameter of interest. In such cases, the prior plays a more significant role in shaping the posterior distribution. If an informative prior is used, the posterior will lean more towards the prior beliefs. As the sample size grows, the influence of the data (likelihood) becomes more dominant in determining the shape and location of the posterior distribution. Even if an informative prior is used, a large amount of data can "override" the prior beliefs. This means that the posterior will be more influenced by the data and less by the prior.
...
```{r}
best_10_movies_with_trials_over_30 <- summary_post_df_all_empbayes %>%
  mutate(mle = num_events / num_trials) %>%
  arrange(desc(mle)) %>%
  head(10)

best_10_movies_with_trials_over_30 %>% select(movie_id, mle, num_trials)

```

The trial size are small, with max size 5.
...
```{r}
worst_10_movies <- summary_post_df_all_empbayes %>%
  mutate(mle = num_events / num_trials) %>%
  arrange(desc(mle)) %>%
  tail(10)

worst_10_movies %>% select(movie_id, mle, num_trials, num_events)
```

The worst 10 movies also have small trial size, with max num_trials 5.
...
```{r}
best_10_movies_by_posterior_mean <- summary_post_df_all_empbayes %>%
  arrange(desc(post_avg)) %>%
  head(10)

best_10_movies_by_posterior_mean %>% select(movie_id, post_avg, num_trials)
```

The trial sizes range from 467 to 25343, which are also large.
...
```{r}
worst_10_movies_by_posterior_mean <- summary_post_df_all_empbayes %>%
  arrange(desc(post_avg)) %>%
  tail(10)

worst_10_movies_by_posterior_mean %>% select(movie_id, post_avg, num_events)
```

We would have some large trail size such as movie_id 8964 with num_trials 4268.
...
The NON-Bayesian method would identify many movies with MLE value equals to 0 or 1, on small sample size, which leads to many BEST or WORST.

The Bayesian method using post_avg for evaluation, and would prefer to identify BEST or WORST movies among those with more reviews without 0 or 1 value.
...
It should be 0.627 instead of 0.53
...
The overall POSITIVE review probability is about 0.627. Therefore, for these movies with 15 reviews, a POSITIVE value greater than 0.627 would suggest a potential **SUPRISE** movie.
